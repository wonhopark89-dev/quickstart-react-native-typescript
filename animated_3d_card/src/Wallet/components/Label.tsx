...
import {useFont, interpolate, Text, useComputedValue} from '@shopify/react-native-skia';
...
export const Label = ({state, y, graphs, width, height}: LabelProps) => {
  ...
  const text = useComputedValue(() => {
    ...
    return format(interpolate(y.current, [0, AJUSTED_SIZE], [graph.data.maxPrice, graph.data.minPrice]));
  }, [y, state]);
  ...
  return (
    <>
      ...
      <Text x={width / 2 - subtitleWidth / 2} y={translateY - 60} text={subtitle} font={subtitleFont} color="#8E8E93" />
    </>
  );
};
