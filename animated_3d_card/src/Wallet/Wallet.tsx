...
import {Canvas, Path, Group, useValue, useComputedValue, LinearGradient, vec} from '@shopify/react-native-skia';
...
export const Wallet = () => {
  ...
  const x = useValue(0);
  const y = useComputedValue(() => getYForX(path.current.toCmds(), x.current), [x, path]);
    ...
    <View style={styles.container}>
      ...
      <Canvas style={{width, height: 2 * height + 30}} onTouch={onTouch}>
        ...
        <Group transform={[{translateY}]}>
          <Path style="stroke" path={path} strokeWidth={2} strokeJoin="round" strokeCap="round">
